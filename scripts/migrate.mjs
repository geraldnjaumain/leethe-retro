...
const controlDatabaseUrl = process.env.DATABASE_ADMIN_URL || process.env.DATABASE_URL;
if (!controlDatabaseUrl) {
  throw new Error("Set DATABASE_ADMIN_URL or DATABASE_URL before running migrations.");
}

const databaseUrls = [...new Set([controlDatabaseUrl, ...catalogShardAdminUrls()])];
const directory = resolve("migrations");
const files = (await readdir(directory)).filter((file) => file.endsWith(".sql")).sort();

async function migrateDatabase(databaseUrl, databaseIndex) {
  ...
  if (lock?.checksum !== lockOwner) {
    throw new Error(`Another database migration is already running on database ${databaseIndex}.`);
  }
...
for (const [databaseIndex, databaseUrl] of databaseUrls.entries()) {
  await migrateDatabase(databaseUrl, databaseIndex);
}
