...
const shardAdminUrls = catalogShardAdminUrls();
const allAdminUrls = [...new Set([adminUrl, ...shardAdminUrls])];
const appUrls = [];
...
setEnv("DATABASE_URL", appUrls[0]);
if (process.env.CATALOG_DATABASE_SHARD_ADMIN_URLS) {
  const shardAppUrls = shardAdminUrls.map((url) => appUrls[allAdminUrls.indexOf(url)]);
  setEnv("CATALOG_DATABASE_SHARD_URLS", shardAppUrls.join(","));
}
await writeFile(envPath, envText);
console.log(
  JSON.stringify({
    level: "info",
    event: "application_role_provisioned",
    databases: allAdminUrls.length,
    role,
  }),
);
